...
const debug_log = (operation: string, code: string, params: Record<string, unknown> = {}): void => {
    ...
    const parts = Object.entries(params).filter(([, v]) => v !== undefined).map(([k, v]) => `${k}:${JSON.stringify(v)}`);
...
export class DB {
    ...
    async update(code: string, query: Document, obj: Document): Promise<unknown> {
        ...
        const { _id, ...without_id } = obj;
        return this.col(code).updateMany(query, { $set: without_id }, { upsert: true });
    }
    ...
    async sum(code: string, query: Document, field: string): Promise<number> {
        ...
        const result = await this.col(code).aggregate([{ $match: query }, { $group: { _id: null, total: { $sum: `$${field}` } } }]).toArray();
